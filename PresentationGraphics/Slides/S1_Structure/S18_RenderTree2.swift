...
struct S18_RenderTree2: View, Slide {
    ...
    var episodes: [Episode] {
        e00 {
            ...
            "Мы получим новое дерево которое назовем RenderTree"
            "Если RenderTree определяет структуру вью для всех состояний"
            "ViewTree определяет дерево для конкретного момента времени"
            "У SwiftUI по факту под капотом есть похожее дерево и оно называется AttributeGraph"
            "На его основе отрисовывается контент на экране"
            "Но для удобства я продолжу использовать название RenderTree т.к. оно понятнее"
